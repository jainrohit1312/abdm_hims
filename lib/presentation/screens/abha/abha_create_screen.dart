...
class _ABHACreateScreenState extends ConsumerState<ABHACreateScreen> {
  ...
  Future<void> _generateOtp() async {
    ...
    try {
      final txnId = await _abdm.generateAadhaarOtp(aadhaar);
      if (!mounted) return;
      setState(() {
        _txnId = txnId;
        _maskedMobile = 'XXXXXX${aadhaar.substring(aadhaar.length - 4)}';
        _currentStep = 1;
        _isGeneratingOtp = false;
      });
      _showSnack('OTP sent to Aadhaar-linked mobile');
    } catch (e) {
      if (!mounted) return;
      setState(() {
        _isGeneratingOtp = false;
  ...
  Future<void> _verifyOtp() async {
    ...
    try {
      final kyc = await _abdm.verifyAadhaarOtp(otp, txnId: _txnId);
      if (!mounted) return;

      final name = (kyc['name'] as String?) ?? '';
      if (_nameController.text.isEmpty) {
        _nameController.text = name.trim();
      }
      if (_mobileController.text.isEmpty) {
        _mobileController.text = (kyc['mobileNumber'] as String?) ?? '';
      }
      final gender = (kyc['gender'] as String?)?.toUpperCase();
  ...
  Future<void> _createAbha() async {
    ...
    if (!_consentGiven) {
      _showSnack('Consent is required to create an ABHA Health ID', isError: true);
      return;
    }
  ...
  Future<void> _downloadCard() async {
    final address = _createdProfile?['abhaAddress'] as String?;
            ...
            content: Column(
              ...
              children: [
                ...
                if (_maskedMobile != null) ...[
                  ...
                  Text(
                    'OTP will be sent to Aadhaar-linked mobile $_maskedMobile',
          ...
          Step(
            ...
            content: Column(
              ...
              children: [
                  ...
                  child: const Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        SizedBox(height: 8),
                        Text(
                          'By creating an ABHA Health ID, you agree to share your health records '
                          'through the Ayushman Bharat Digital Mission (ABDM) platform. '
                          'Your data will be encrypted and shared only with your explicit consent.',
                        ),
                      ],
                    ),
                  ),
                ...
                CheckboxListTile(
                  title: const Text('I consent to the creation of ABHA Health ID'),
                  value: _consentGiven,
